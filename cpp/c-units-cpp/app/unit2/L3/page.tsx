        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#020617]/90 backdrop-blur-md border-b border-slate-800 z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-white text-sm md:text-base leading-tight">Pointers and Classes</h1>
            ...
            <section id="intro" className="pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
                ...
                <p className="text-xl text-slate-400 leading-relaxed max-w-3xl mb-12">
                    Welcome to Unit 2, Lecture 3. This interactive note will cover the foundational concepts for this topic.
                </p>
            </section>
            ...
            <section id="topics" className="px-6 md:px-12 max-w-7xl mx-auto mb-24">
                ...
                <TheoryCard title="Detailed Lecture Content" icon={<BookOpen size={18} />} variant="purple">
                    ...
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Classes containing pointers</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">When a class contains a pointer as a data member, special care must be taken. Dynamic memory is usually allocated in the constructor and deallocated in the destructor. Default shallow copying will cause double-free errors, so a custom copy constructor and assignment operator (Deep Copy) are strictly required (The Rule of Three).</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Pointer to objects</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">You can create pointers that point to class objects: `ClassName *ptr = new ClassName();`. Instead of the dot operator (`.`), the arrow operator (`-&gt;`) is used to access the object&apos;s member functions and variables through the pointer.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">this pointer</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">The `this` pointer is a hidden, implicit pointer passed to all non-static member functions. It points to the object that invoked the function. It is essential when local variable names shadow class member names, allowing you to explicitly refer to the member as `this-&gt;variable`.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Pointer to data member</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">C++ allows pointers to specifically point to class members rather than specific instances. Declared as `DataType ClassName::*ptr`, it represents an offset rather than an absolute address. It is accessed using the `.*` or `-&gt;*` operators along with an actual object instance.</p>
                    </div>
                </TheoryCard>
            </section>
        </div>
