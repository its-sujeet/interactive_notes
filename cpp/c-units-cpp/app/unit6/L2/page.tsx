        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#020617]/90 backdrop-blur-md border-b border-slate-800 z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-white text-sm md:text-base leading-tight">Exception Mechanisms</h1>
            ...
            <section id="intro" className="pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
                <div className="inline-flex items-center gap-2 bg-blue-900/20 border border-blue-500/30 text-blue-300 px-3 py-1 rounded-full text-xs font-bold mb-6">
                    <Terminal size={14} /> Exception Handling, Templates and Standard Template Library (STL)
                </div>
                ...
                <p className="text-xl text-slate-400 leading-relaxed max-w-3xl mb-12">
                    Welcome to Unit 6, Lecture 2. This interactive note will cover the foundational concepts for this topic.
                </p>
            </section>
            ...
            <section id="topics" className="px-6 md:px-12 max-w-7xl mx-auto mb-24">
                ...
                <TheoryCard title="Detailed Lecture Content" icon={<BookOpen size={18} />} variant="purple">
                    ...
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Throwing mechanism</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">When an error condition is detected, the `throw` keyword is used to throw an object, variable, or literal (e.g., `throw runtime_error(&quot;Error&quot;);` or `throw 404;`). The program instantly stops current execution and unwinds the stack looking for a matching catch block.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Catching mechanism</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">A `catch` block catches thrown exceptions matching its specified type. You can have multiple catch blocks for a single try block to handle different data types (int, strings, custom classes). A catch-all block `catch(...)` catches any exception regardless of type.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Rethrowing an exception</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">If a catch block cannot completely handle an exception or wants to log it and pass it up the call stack, it can rethrow the exception using the `throw;` keyword with no arguments. The exception is then caught by the next enclosing try-catch block.</p>
                    </div>
                </TheoryCard>
            </section>
        </div>
