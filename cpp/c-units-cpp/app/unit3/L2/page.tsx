        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#020617]/90 backdrop-blur-md border-b border-slate-800 z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-white text-sm md:text-base leading-tight">File Read/Write and Access Modes</h1>
            ...
            <section id="intro" className="pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
                <div className="inline-flex items-center gap-2 bg-blue-900/20 border border-blue-500/30 text-blue-300 px-3 py-1 rounded-full text-xs font-bold mb-6">
                    <Terminal size={14} /> Data File Operations and Constructors
                </div>
                <h1 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-white mb-6 leading-tight">
                    File Read/Write and Access Modes
                </h1>
                <p className="text-xl text-slate-400 leading-relaxed max-w-3xl mb-12">
                    Welcome to Unit 3, Lecture 2. This interactive note will cover the foundational concepts for this topic.
                </p>
            </section>
            ...
            <section id="topics" className="px-6 md:px-12 max-w-7xl mx-auto mb-24">
                ...
                <TheoryCard title="Detailed Lecture Content" icon={<BookOpen size={18} />} variant="purple">
                    ...
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Reading/Writing of files</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">Text files are read using the extraction operator `&gt;&gt;` (which skips whitespace) or `getline()`. They are written using the insertion operator `&lt;&lt;`. For complex text parsing, data is extracted into appropriate primitive types directly from the file stream.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Sequential access and random access file processing</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">Sequential access reads data from beginning to end. Random access allows jumping to specific locations in the file. C++ provides `seekg()` and `seekp()` to move the internal read and write pointers, and `tellg()` and `tellp()` to get their current positions.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Binary file operations</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">Binary files store data exactly as it appears in memory, avoiding character translation overhead. They are opened with the `ios::binary` mode. Data is read and written using the `read()` and `write()` methods, passing the memory address cast to a `char*` and the byte size.</p>
                    </div>
                </TheoryCard>
            </section>
        </div>
