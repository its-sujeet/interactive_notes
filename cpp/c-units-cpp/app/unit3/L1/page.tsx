        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#020617]/90 backdrop-blur-md border-b border-slate-800 z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-white text-sm md:text-base leading-tight">File Fundamentals</h1>
            ...
            <section id="intro" className="pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
                ...
                <p className="text-xl text-slate-400 leading-relaxed max-w-3xl mb-12">
                    Welcome to Unit 3, Lecture 1. This interactive note will cover the foundational concepts for this topic.
                </p>
            </section>
            ...
            <section id="topics" className="px-6 md:px-12 max-w-7xl mx-auto mb-24">
                ...
                <TheoryCard title="Detailed Lecture Content" icon={<BookOpen size={18} />} variant="purple">
                    ...
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Opening and closing of files</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">File handling requires the `&lt;fstream&gt;` header. `ifstream` is used for reading, `ofstream` for writing, and `fstream` for both. A file is opened using the `open()` method or constructor, and must be closed using the `close()` method to flush buffers and release system resources.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Modes of file</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">File modes define how a file is opened. Common modes include `ios::in` (read), `ios::out` (write, overwrites file), `ios::app` (append to end), `ios::trunc` (truncate existing), and `ios::binary` (open in binary mode). Multiple modes can be combined using the bitwise OR operator `|`.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">File stream functions</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">File streams inherit from standard streams, meaning you can use `&lt;&lt;` and `&gt;&gt;`. Additionally, `getline()` reads entire lines, `eof()` checks for the end of the file, `get()` reads single characters, and `put()` writes single characters.</p>
                    </div>
                </TheoryCard>
            </section>
        </div>
