        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-20">
            <header className="fixed top-0 left-0 right-0 h-16 bg-[#020617]/90 backdrop-blur-md border-b border-slate-800 z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-white text-sm md:text-base leading-tight">Dynamic Memory</h1>
            ...
            <section id="intro" className="pt-32 pb-16 px-6 md:px-12 max-w-7xl mx-auto">
                ...
                <p className="text-xl text-slate-400 leading-relaxed max-w-3xl mb-12">
                    Welcome to Unit 5, Lecture 1. This interactive note will cover the foundational concepts for this topic.
                </p>
            </section>
            ...
            <section id="topics" className="px-6 md:px-12 max-w-7xl mx-auto mb-24">
                ...
                <TheoryCard title="Detailed Lecture Content" icon={<BookOpen size={18} />} variant="purple">
                    ...
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Dynamic memory allocation using new and delete operators</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">Memory is allocated at runtime on the Heap using `new`. `int *ptr = new int;` or `new int[10];`. Unlike C&apos;s malloc, `new` automatically calls the constructor. Memory must be explicitly freed using `delete ptr;` or `delete[] arr;` which automatically calls destructors.</p>
                    </div>
                    <div className="mb-8 p-6 bg-slate-900/50 border border-slate-700 rounded-xl">
                        <h3 className="text-xl font-bold text-blue-300 mb-3 border-b border-slate-800 pb-2">Memory leak and allocation failures</h3>
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base">A memory leak occurs when dynamically allocated memory is never deleted, permanently losing heap space until the program terminates. Allocation failures occur when the heap is full, causing `new` to throw a `std::bad_alloc` exception.</p>
                    </div>
                </TheoryCard>
            </section>
        </div>
